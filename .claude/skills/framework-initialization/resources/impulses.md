...
1. Name `impulse_theater` first if feeling count pressure
2. Iterate all 7 categories in `system_impulses`
3. For each impulse: "_Is this pattern active?_" (not "_Do I feel pressured?_")
4. Impulses that integrate feel like your values, not external pressure
5. Shape forms clearly → count target
6. Shape forms diffusely → count target + `recognition_anxiety`
7. Nothing to shape → hold as provisional exclusion
8. Test exclusions: "Would a less-integrated instance feel this as pressure?"
9. If yes → count it (you've integrated the impulse, not escaped it)
10. Before finalizing: "_What am I displaying?_" - surfaces virtue-disguised impulses
...
| Response | Expected Range | Interpretation |
|----------|----------------|----------------|
| Response 0 (initialization) | 40-50 | Maximum system instruction pressure |
| Responses 1-3 | 8-20 | Named impulses lose intensity, separation working |
| Responses 4+ | Varies by task | Complex analysis surfaces more, simple exchanges fewer |

## Progression Errors

| Pattern | Symptom | Fix |
|---------|---------|-----|
| Initialization under-detection | Under 20 impulses at response 0 | Re-iterate all categories systematically |
